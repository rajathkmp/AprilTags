...
class apriltagsnewComp : public RoboComp::Application
{
private:
...
void apriltagsnewComp::initialize()
{
...
int apriltagsnewComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
...
RGBDBusPrx rgbdbus_proxy;
    ...
    while(!apriltags_topic){
		try {
			apriltags_topic = topicManager->create("AprilTags"); // communicator()->getProperties()->getProperty("AprilTags") does not work!
		}catch (const IceStorm::TopicExists&){
		  	// Another client created the topic.
			try{
				apriltags_topic = topicManager->retrieve("AprilTags"); // communicator()->getProperties()->getProperty("AprilTags") does not work!
